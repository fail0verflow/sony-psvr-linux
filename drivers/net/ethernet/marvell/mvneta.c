...
#define MVNETA_TXQ_CMD_1                         0x3e00
...
#define MVETH_TXQ_TOKEN_COUNT_REG(q)             (0x3e60 + ((q) << 2))
#define MVNETA_TX_REFILL_PERIOD_REG(q)			 0x3e08
...
#define MODEL_88E1510 0xdd0
#define MODEL_MASK 0xfff0
#define GE_PHY_ADD0		1
#define GE_RGMII_MEM_RSTN	0
#define GE_RGMII_RSTN		1

#define IA_Gbl_PERIF		0x480
#define RA_PERIF_FUNC_SEL	0x001C
#define FUNC_SEL_GMAC_ETH	1
#define RA_Gbl_STICKY_RSTN	0x010C
...
/* MARVELL SOC GMAC AXI init */
static void mvneta_AXI_init(struct mvneta_port *pp)
{
	u32 val;

	val = readl(MEMMAP_CHIP_CTRL_REG_BASE + IA_Gbl_PERIF + RA_PERIF_FUNC_SEL);
	writel(val | (1 << FUNC_SEL_GMAC_ETH),
		MEMMAP_CHIP_CTRL_REG_BASE + IA_Gbl_PERIF + RA_PERIF_FUNC_SEL);

	val = readl((MEMMAP_CHIP_CTRL_REG_BASE + RA_Gbl_STICKY_RSTN) & 0xFFFFFFFC);
	writel(val | (1 << GE_RGMII_MEM_RSTN),
		(MEMMAP_CHIP_CTRL_REG_BASE + RA_Gbl_STICKY_RSTN) & 0xFFFFFFFC);
	udelay(20);
	writel(val | (1 << GE_RGMII_MEM_RSTN) | (1 << GE_RGMII_RSTN),
		(MEMMAP_CHIP_CTRL_REG_BASE + RA_Gbl_STICKY_RSTN) & 0xFFFFFFFC);
	udelay(1000);

	mvreg_write(pp, MVNETA_UNIT_CONTROL, 0x01000000);
	udelay(100);
	mvreg_write(pp, MVNETA_UNIT_CONTROL, 0x00000000);
	udelay(100);

	/* SMI address */
	mvreg_write(pp, MVNETA_PHY_ADDR, GE_PHY_ADD0);

	/* increase pad drive strength */
	writel(0xffffffff, 0xF7EA0170);
	writel(0xffffffff, 0xF7EA0174);
	writel(0xffffffff, 0xF7EA01E4);
}
...
static void mvneta_port_down(struct mvneta_port *pp)
{
	...
	while (1) {
		...
		/*double check*/
		val = mvreg_read(pp,MVNETA_PORT_STATUS);
		if ((val & MVNETA_TX_FIFO_EMPTY) && !(val & MVNETA_TX_IN_PRGRS))
			break;
		else
			netdev_warn(pp->dev,
					    "TX FIFO empty timeout status=0x08%x\n",
...
static void mvneta_defaults_set(struct mvneta_port *pp)
{
	...
	mvreg_write(pp, MVNETA_TX_REFILL_PERIOD_REG(queue), 0x9A);
			   ...
			   struct mvneta_tx_queue *txq)
{
	...
	mvreg_write(pp, MVETH_TXQ_TOKEN_COUNT_REG(txq->id), 0xffffffff);
...
static int mvneta_open(struct net_device *dev)
{
	...
	/*read phy Model Number*/
	model = mdiobus_read(pp->phy_dev->bus, GE_PHY_ADD0, MII_PHYSID2);
	if ((model & MODEL_MASK) ==  MODEL_88E1510) {
		mdiobus_write(pp->phy_dev->bus, GE_PHY_ADD0, 22, 0x2);
		mdiobus_write(pp->phy_dev->bus, GE_PHY_ADD0, 21, 0x1046);
		mdiobus_write(pp->phy_dev->bus, GE_PHY_ADD0, 22, 0);
		mdiobus_write(pp->phy_dev->bus, GE_PHY_ADD0, MII_BMCR, 0x9140);
	}
...
{
	mvreg_write(pp, 0x2290, 0);
	mvreg_write(pp, 0x2294, 0x0FFF);
	/* base win 0 address register -> attribut */
	mvreg_write(pp, 0x2200, 0x00000e00);
	/* 4 GB */
	mvreg_write(pp, 0x2204, 0xffff0000);
	/* base window 1 */
	mvreg_write(pp, 0x2208 , 0x00000b00);
	mvreg_write(pp, 0x220c , 0x00000000);
	/* base win 2 */
	mvreg_write(pp, 0x2210 , 0x00000e00);
	mvreg_write(pp, 0x2214 , 0x00000000);
	/* base win 3 */
	mvreg_write(pp, 0x2218 , 0x00000d00);
	mvreg_write(pp, 0x221c , 0x00000000);
	/* base win 4 */
	mvreg_write(pp, 0x2220 , 0x00000e00);
	mvreg_write(pp, 0x2224 , 0x0000000);
	/* base win 5 */
	mvreg_write(pp, 0x2228 , 0x00000b00);
	mvreg_write(pp, 0x222c , 0x00000000);

	printk("Network controller revision: %04x\n", mvreg_read(pp, 0x25BC));
}

/* Power up the port */
static void mvneta_port_power_up(struct mvneta_port *pp)
{
	u32 val;
	int phy_mode = pp->phy_interface;
	...
	mvreg_write(pp, MVNETA_GMAC_AUTONEG_CONFIG, 0x904a);
